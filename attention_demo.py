...
from models import FCNAttention
...
BATCH_SIZE = 8
NUM_EPOCHS = 5
NUM_SAMPLES = 10000  # Using same number of images as the baseline for fair comparison
...
def train_attention_demo():
    ...
    val_dataset.valid_pairs = val_dataset.valid_pairs[:min(100, len(val_dataset.valid_pairs))]  # Using more validation samples
    ...
    # Create model - using the attention-enhanced version
    model = FCNAttention(
        num_classes=NUM_CLASSES,
        backbone="resnet18",  # Using same backbone as baseline for fair comparison
        pretrained=True
    ).to(device)
...
def visualize_predictions(model, dataloader, class_mapping, num_samples=5):
    ...
    with torch.no_grad():
        for i, (images, masks) in enumerate(dataloader):
            ...
            for j in range(images.size(0)):
                ...
                plt.tight_layout()
                plt.savefig(f'attention_output/sample_{img_idx}.png')
                plt.close()
...
if __name__ == "__main__":
    model, class_mapping = train_attention_demo()
    print("Attention model demo completed!") 